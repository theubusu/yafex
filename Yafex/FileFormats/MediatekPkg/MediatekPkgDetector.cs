...
public class MediatekPkgDetector : IFormatDetector
{
    ...
    private static bool IsDecryptedHeader(ReadOnlySpan<byte> bytes)
    {
        var hdr = SpanEx.Cast<PkgHeader>(bytes)[0];
        return hdr.MtkMagic == PkgHeader.MTK_MAGIC;
    }

    private bool TryDecryptHeaderStandard(Memory<byte> bytes, [MaybeNullWhen(false)] out Memory<PkgHeader> header)
    {
    ...
    private bool TryDecryptHeaderNew(Memory<byte> bytes, [MaybeNullWhen(false)] out Memory<PkgHeader> header)
    {
